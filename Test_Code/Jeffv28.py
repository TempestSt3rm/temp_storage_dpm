...
class Robot:
    ...
    def consolidate_movement(self):
        ...
        if len(self.movement_queue) == 0:
            return
        new_movement.append(self.movement_queue[0])
        for i in range(1,len(self.movement_queue)):
            old_direction, old_distance = self.movement_queue[i]
            new_direction, new_distance = new_movement[0]
            if new_direction == old_direction:
                new_movement.pop()
                combined_distance = old_distance+new_distance
                new_movement.append((old_direction,combined_distance))
            new_movement.append((old_direction,old_distance))
        self.movement_queue = new_movement
    ...
    def move_forward(self, distance):
        ...
        try:
            ...
            while traveled_distance < distance:
                ...
                traveled_distance += 0.1 * 5  # Convert time to distance (adjust scale factor if needed)
        finally:
