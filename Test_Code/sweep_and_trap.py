...
class Robot:
    ...
    def sweepAndDetect(self):
        ...
        if (angle != -500):
            ...
            print("we found the block at angle: " + str(blockAngle))
    ...
    def capture_block(self):
        ...
        self.rotateGateUp()
        while not self.is_block_present():
            self.move_forward(2)
        if self.is_Poo():
            self.move_forward(5)
        else:
    ...
    def move_forward(self, distance):
        ...
        try:
            ...
            self.movement_arr.append(("f", traveled_distance))
            self.motor_left.set_power(0)
            self.motor_right.set_power(0)
    ...
    def move_backwards(self,distance):
        ...
        try:
            ...
            self.movement_arr.append(("b", traveled_distance))
            self.motor_left.set_power(0)
            self.motor_right.set_power(0)
    ...
    def rotate_robot(self, direction, target_angle):
        ...
        turn_direction = direction[0].lower()
        if turn_direction == "l":
        ...
        while True:
            ...
            if diff >= target_angle:
                break
        self.movement_arr.append((turn_direction,target_angle))
        self.motor_left.set_power(0)
        self.motor_right.set_power(0)


    def go_home(self):
        conversion_dict = {"l":"r","r":"l","f":"b","b":"f"}
        
        
        while len(self.movement_arr) != 0:
            old_direction,old_distance = self.movement_arr.pop()
            print("old direction " + old_direction) 
            curr_direction = conversion_dict[old_direction.lower()]
            print(f"moving in direction {curr_direction} for distance: {old_distance}")
            if curr_direction == "l" or curr_direction == "r":
                self.rotate_robot(curr_direction,old_distance)
            elif curr_direction == "f":
                self.move_forward(old_distance)
            else:
                self.move_backwards(old_distance)
            self.movement_arr.pop()
...
if __name__ == "__main__":
    ...
    try:
        ...
        angle = robot.sweepAndDetect()
        while angle == -500:
            robot.do_math_on_angle(angle)
            angle = robot.sweepAndDetect()
        robot.capture_block()
        #robot.move_forward(40)
        #robot.rotate_robot("l",40)
        #robot.move_forward(20)
        #robot.rotateRight("r",40)
        #robot.go_home()
